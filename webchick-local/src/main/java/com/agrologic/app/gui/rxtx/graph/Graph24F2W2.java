 ...
 */
public class Graph24F2W2 extends AbstractGraph{
    public Graph24F2W2(GraphType type, String values, Long currentTime, Locale locale) {
        super(type, values);
    ...
    public final JFreeChart createChart() {
        if (!isEmpty()) {
            DateAxis dateaxis = new DateAxis(dictionary.get("graph.f2w2.axis.time"));
            ...
            NumberAxis numberaxis = new NumberAxis(dictionary.get("graph.f2w2.axis.feed"));
            ...
            NumberAxis waterAxis = new NumberAxis(dictionary.get("graph.f2w2.axis.water"));
            ...
            // set tooltip
//            chart = new JFreeChart(dictionary.get("graph.f2w2.title"), JFreeChart.DEFAULT_TITLE_FONT, plot, true);
            chart = new JFreeChart(dictionary.get("graph.f2w2.title"), JFreeChart.DEFAULT_TITLE_FONT, plot, true);
            ...
            chart = new JFreeChart(dictionary.get("graph.f2w2.title"), JFreeChart.DEFAULT_TITLE_FONT, xyplot, true);
        }
...
//        final TimeSeries feedSeries = new TimeSeries(dictionary.get("graph.f2w2.series.feed"));
        final TimeSeries feedSeries = new TimeSeries("Feed");
        int hr = (int) (currentTime / 100) - 1;
//        int hr = (int) (currentTime / 100);

        for (int i = FEED_2_CONS + DAY_HOURS - 1; i >= FEED_2_CONS; i--, hr--) {
    ...
    private XYDataset createWaterDataset() {
        ...
        Day yesterday = new Day(SerialDate.createInstance(yday.getDate(), yday.getMonth(), yday.getYear()));
        final TimeSeriesCollection timeSeriesCollection = new TimeSeriesCollection();
        final TimeSeries timeseries1 = new TimeSeries(dictionary.get("graph.f2w2.series.water"));
