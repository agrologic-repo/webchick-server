 ...
 */
public class Graph24FWPB extends AbstractGraph {
    public Graph24FWPB(GraphType type, String values, Long currentTime, Locale locale) {
        super(type, values);
    ...
    public final JFreeChart createChart() {
        if (!isEmpty()) {
            DateAxis dateaxis = new DateAxis(dictionary.get("graph.fwpb.axis.time"));
            ...
            NumberAxis numberaxis = new NumberAxis(dictionary.get("graph.fwpb.axis.feed"));
            ...
            NumberAxis waterAxis = new NumberAxis(dictionary.get("graph.fwpb.axis.water"));
//            NumberAxis tempAxis = new NumberAxis(dictionary.get("graph.ioh.axis.temperature"));
            ...
            // set tooltip
//            chart = new JFreeChart(dictionary.get("graph.fwpb.title"), JFreeChart.DEFAULT_TITLE_FONT, plot, true);
            chart = new JFreeChart(dictionary.get("graph.fwpb.title"), JFreeChart.DEFAULT_TITLE_FONT, plot, true);
            ...
            chart = new JFreeChart(dictionary.get("graph.fwpb.title"), JFreeChart.DEFAULT_TITLE_FONT, xyplot, true);
        }
    ...
    private XYDataset createFeedDataset() {
        ...
        final TimeSeries feedSeries = new TimeSeries(dictionary.get("graph.fwpb.series.feed"));
...
//        int hr = (int) (currentTime / 100);
    ...
    private XYDataset createWaterDataset() {
        ...
        Day yesterday = new Day(SerialDate.createInstance(yday.getDate(), yday.getMonth(), yday.getYear()));
        final TimeSeriesCollection timeSeriesCollection = new TimeSeriesCollection();
        final TimeSeries timeseries1 = new TimeSeries(dictionary.get("graph.fwpb.series.water"));
