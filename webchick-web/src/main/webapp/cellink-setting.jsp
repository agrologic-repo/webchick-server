                                ...
                                <table style="border-left-width: 0px; border-top-width: 0px; border-bottom-width: 0px; border-right-width: 0px; border-style: solid; border-collapse: collapse;"width="100%">
                                        ...
                                        <td>
                                            <table class="table-list" border="1" width="100%" cellpadding="2" cellspacing="1">
                                                ...
                                                <tr>
                                                    ...
                                                    <td bgcolor="#F3F3F3">
                                                        <%=editCellink.getPassword() %>
                                                    </td>
                                                    <td style="font-weight:bold">
                                                        User ID
                                                    </td>
                                                    <td bgcolor="#F3F3F3">
                                                        <%=editCellink.getUserId() %>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td style="font-weight:bold">
                                                    ...
                                                    <td bgcolor="#F3F3F3">
                                                        <%=editCellink.getIp() %>: <%=editCellink.getPort() %>
                                                    </td>
                                                    <td style="font-weight:bold">
                                                        SIM Number
                                                    </td>
                                                    <td bgcolor="#F3F3F3">
                                                        <input type="text" name="simnumber" value=" <%=editCellink.getSimNumber() %>"/>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td style="font-weight:bold">
                                                    ...
                                                    <%--style="font-weight: bold;color:<%=CellinkState.getCellinkStateColor(editCellink.getState()) %>"--%>
                                                         <%=editCellink.getCellinkState()%>
                                                    </td>
                                                </tr>
                                                </tbody>
                                            </table>
                                    ...
                                    <tr>
                                        <td colspan="2" width="100%">
                                            <h2><%=session.getAttribute("table.caption.controller")%></h2>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td width="100%">
                                            <input class="button" type="button" value="<%=session.getAttribute("button.add.controller")%>" onclick="addController(<%=editCellink.getUserId() %> , <%=editCellink.getId()%>);"/>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td width="100%">
                                            <b><%=editCellink.getControllers().size()%></b>&nbsp;<%=session.getAttribute("label.records")%>
                                            <table class="table-list" border="1">
                                                <thead>
                                                <tr>
                                                    <th align="center" nowrap>
                                                        <%=session.getAttribute("table.col.controller.title")%>
                                                    </th>
                                                    <th align="center" nowrap>
                                                        <%=session.getAttribute("table.col.controller.name")%>
                                                    </th>
                                                    <th align="center" nowrap>
                                                        <%=session.getAttribute("table.col.controller.netname")%>
                                                    </th>
                                                    <th align="center" nowrap>
                                                        <%=session.getAttribute("table.col.controller.progversion")%>
                                                    </th>
                                                    <th align="center" nowrap>
                                                        <%=session.getAttribute("table.col.controller.active")%>
                                                    </th>
                                                ...
                                                <tr class="even" onMouseOver="changeEven(this);" onmouseout="changeEven(this)">
                                                    <%}%>
                                                    <td align="center">
                                                        <a href="./editcontrollerrequest.html?userId=<%=editCellink.getUserId() %>&cellinkId=<%=controller.getCellinkId() %>&controllerId=<%=controller.getId()%>"><%=controller.getTitle()%></a>
                                                    </td>
                                                    <td align="center">
                                                        <%=controller.getName()%>
                                                    </td>
                                                    ...
                                                    <td align="center">
                                                        ...
                                                        <input type="checkbox" name="Nactive" id="Nactive" checked onclick="activate(<%=editCellink.getUserId()%>, <%=cellinkId%>, <%=controller.getId() %>, 'OFF')"/>
                                                        <%} else {%>
                                                        <input type="checkbox" name="Nactive" id="Nactive" onclick="activate(<%=editCellink.getUserId()%>, <%=cellinkId%>, <%=controller.getId() %>, 'ON')"/>
                                                        <%}%>
                                                    </td>
                                                    <td align="center">
                                                        <img src="resources/images/edit.gif" style="cursor: pointer" border="0" hspace="5"/>
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <%} else {%>
                                            <button name="btnCancel" type="button" onclick='return back("./overview.html?editCellink.getUserId()=<%=editCellink.getUserId() %>");'>
