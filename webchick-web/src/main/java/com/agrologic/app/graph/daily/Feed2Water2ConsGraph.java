...
public class Feed2Water2ConsGraph extends PerHourReportGraph{
    public Feed2Water2ConsGraph(String values, Long currentTime, Locale locale) {
    ...
    public final JFreeChart createChart() {
        ...
        if (!isEmpty()) {
            ...
            numberaxis = new NumberAxis(dictinary.get("graph.feed2.water2.axis.feed2"));
            ...
            waterAxis = new NumberAxis(dictinary.get("graph.feed2.water2.axis.water2"));
            numberaxis.setStandardTickUnits(NumberAxis.createIntegerTickUnits());
            ...
            // set tooltip
            chart = new JFreeChart(dictinary.get("graph.feed2.water2.title"), JFreeChart.DEFAULT_TITLE_FONT, plot, true);
            ...
            chart = new JFreeChart(dictinary.get("graph.feed2.water2.title"), JFreeChart.DEFAULT_TITLE_FONT, xyplot, true);
        }
        return chart;
    }
    ...
    private XYDataset createFeedDataset() {
        ...
        feedSeries = new TimeSeries(dictinary.get("graph.feed.water.series.feed2"));
        hour = (int) (currentTime / 100) - 1;
//        hour = (int) (currentTime / 100);
        for (int i = FEED_2_CONS + DAY_HOURS - 1; i >= FEED_2_CONS; i--, hour--) {
