 ...
 */
public class FeedWaterPerBirdGraph extends PerHourReportGraph {
    public FeedWaterPerBirdGraph(String values, Long currentTime, Locale locale) {
        super(values);
    ...
    public final JFreeChart createChart() {
        ...
        if (!isEmpty()) {
            ...
            numberaxis = new NumberAxis(dictinary.get("graph.feed.water.axis.feed.per.bird"));
            ...
            waterAxis = new NumberAxis(dictinary.get("graph.feed.water.axis.water.per.bird"));
            numberaxis.setStandardTickUnits(NumberAxis.createIntegerTickUnits());
            ...
            // set tooltip
            chart = new JFreeChart(dictinary.get("graph.feed.water.per.bird.title"), JFreeChart.DEFAULT_TITLE_FONT, plot, true);
            ...
            chart = new JFreeChart(dictinary.get("graph.feed.water.per.bird.title"), JFreeChart.DEFAULT_TITLE_FONT, xyplot, true);
        }
        return chart;
    }
    ...
    private XYDataset createFeedDataset() {
        ...
        feedSeries = new TimeSeries(dictinary.get("graph.feed.water.series.feed.per.bird"));
//        hour = (int) (currentTime / 100) - 1;
        hour = (int) (currentTime / 100);
        for (int i = FEED_CONS_PER_BIRD + DAY_HOURS - 1; i >= FEED_CONS_PER_BIRD; i--, hour--) {
    ...
    private XYDataset createWaterDataset() {
        ...
        yesterday = new Day(SerialDate.createInstance(yday.getDate(), yday.getMonth(), yday.getYear()));
        timeSeriesCollection = new TimeSeriesCollection();
        timeseries1 = new TimeSeries(dictinary.get("graph.feed.water.series.water.per.bird"));
        resetMinMaxY();
//        int hr = (int) (currentTime / 100) - 1;
        hr = (int) (currentTime / 100);//test
        for (int i = WATER_CONS_PER_BIRD + DAY_HOURS - 1; i >= WATER_CONS_PER_BIRD; i--, hr--) {
            value = DataFormat.formatToStringValue(DataFormat.DEC_0, Long.valueOf(datasetString[i]));
            intValue = Integer.valueOf(value);
