...
public class HistoryContentCreator {
     ...
     */
    public static HistoryContent createPerDayHistoryContent(final TreeMap<Integer, String> historyDataByGrowDayMap,
                                                            List<Data> perDayHistoryList, HistoryContent historyContent) {

        TreeMap<Integer, List<Data>> histPerDay = (TreeMap) historyContent.getHistoryContentPerDay();
        Integer startGrowDay = 1;
        if(!histPerDay.isEmpty()) {
            startGrowDay = histPerDay.lastKey();
        }
        SortedMap<Integer,String> workingTree = historyDataByGrowDayMap.tailMap(startGrowDay);
        List<Data> newPerDayHistoryList = new ArrayList<Data>();
        for (Map.Entry<Integer, String> entry : workingTree.entrySet()) {
